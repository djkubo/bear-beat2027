...
- **Middleware y sesión en /admin:** Para /admin no se redirige desde el middleware: se deja pasar y el layout (Node) decide. El layout usa `createServerClient` de `src/lib/supabase/server.ts`, que **debe** incluir `cookies.get` y `cookies.remove` además de `getAll`/`setAll`; sin `get`, la librería no puede leer la sesión fragmentada y `getUser()` devuelve null. En `server.ts` se pasan `get`, `set`, `remove` al cliente Supabase.
- **API /api/videos y caché:** La ruta tiene `dynamic = 'force-dynamic'` y cabecera `Cache-Control: no-store` para que tras un sync FTP los números (totalVideos, genreCount) se actualicen de inmediato en la landing y /contenido; sin esto Next o el navegador podrían servir respuestas cacheadas.
- **API /api/videos y límite Supabase:** Supabase devuelve por defecto como máximo 1000 filas por SELECT. Si el pack tiene más de 1000 videos (ej. 1077 tras sync FTP), hay que usar `.limit(10000)` en la query de `videos` para que la API devuelva el total real; si no, la landing y /contenido seguirían mostrando 1000.

### 19.8 Cambios del 30 ene 2026 (scroll, chat, home datos reales, logos compatibles)
- **Scroll vertical:** En `src/app/globals.css` se añadió `overflow-y: auto !important` en `html` y `body` en todos los bloques donde se usa `overflow-x: hidden`, para que la web permita scroll vertical. Se eliminó la regla `* { max-width: 100vw }` y se reemplazó por `body, main { max-width: 100vw }` para no romper modales/dropdowns ni el flujo del documento. En `src/app/layout.tsx` se añadió la clase `overflow-y-auto` a `body` y `main`.
- **Chat a la derecha y sin tapar CTAs:** En `src/components/chat/ChatWidget.tsx` el widget pasó de `left-4` a `right-4`, de `items-start` a `items-end`, de `z-50` a `z-40` (para que quede por debajo del navbar donde está "Acceso Total $350") y de `bottom-4` a `bottom-6` para separarlo de barras fijas en móvil.
- **Home: datos reales y coherencia:** En `src/components/landing/HomeLanding.tsx` se eliminó el fallback inventado "3,200" para el número de videos; ahora se usa solo `packInfo` o `inventory` (API `/api/videos`). En el hero, si no hay datos se muestra "…" (cargando) o "miles de" (catálogo vacío). Se añadió `StatsSection` con props `totalPurchases` y `totalVideos` para mostrar "+X DJs" y texto de apoyo con datos reales. El modal de demo (DemoPlayer) recibe `totalVideos` y el botón muestra "DESBLOQUEAR ESTE Y [número real] MÁS". La sección de demos tiene estado de carga (skeleton) y mensaje cuando el catálogo está vacío. Textos unificados: "Pack Enero 2026", "100% transparencia".
- **Logos "Compatible con" (Serato, Rekordbox, VirtualDJ, etc.):** Nuevo componente `src/components/landing/compatible-logos.tsx` que muestra logos desde `/public/logos/` con nombres `serato.png`/`.svg`, `rekordbox.png`/`.svg`, `virtualdj.png`/`.svg`, `pioneer-dj.png`/`.svg`, `denon-dj.png`/`.svg`. Intenta primero `.svg` y luego `.png`; si falla, muestra el nombre de la marca en texto. Se usa en `StatsSection` (5 marcas, altura 44px) y en el hero de HomeLanding (3 marcas, variante `hero`, altura 36px). Instrucciones para añadir logos oficiales en `public/logos/README_COMPATIBLES.md` (enlaces a Serato Resources, Pioneer DJ, etc.).
