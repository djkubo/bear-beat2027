...
- **Middleware y sesión en /admin:** El middleware debe pasar a Supabase `cookies.get` (además de `getAll` y `setAll`) para que la librería pueda leer la sesión fragmentada; sin `get`, `getUser()` devuelve siempre null y se redirige a login al ir a /admin. En `src/middleware.ts` se incluyen `get`, `set` y `remove` en el objeto `cookies` pasado a `createServerClient`.
